...
const addMoney = async (payload: ITransaction, decodedToken: JwtPayload) => {
  ...
  try {
    ...
    const data: PaymentData = {
      ...
      success_url: `${envVars.BACKEND_URL}${envVars.SSL.SUCCESS_URL}?trans_id=${transaction[0]._id}`,
      fail_url: `${envVars.BACKEND_URL}${envVars.SSL.FAIL_URL}?trans_id=${transaction[0]._id}`,
      cancel_url: `${envVars.BACKEND_URL}${envVars.SSL.CANCEL_URL}?trans_id=${transaction[0]._id}`,
      shipping_method: "N/A",
